...
int mline(double dist, double speed,int time);
...
enum {ms_init,ms_fwd,ms_turn, ms_line, ms_end};
...
FILE *lines;
char outfile[] = "lines.dat";


lines = fopen(outfile, "w");
for(int i = 0;i < linesensor->length;i++){
	fprintf("%d ",linesensor->data[i]);
}
	fprintf("\n");
...
*/
   ...
   switch (mission.state) {
     case ms_init:
       n=4; dist=1.5;angle=-0.5/180*M_PI;
     ...
     case ms_line:
	if (mline(dist,0.6,mission.time))  mission.state=ms_end;
     break;
...
if (p->cmd !=0){
     ...
     case mot_stop:
       ...
       case mot_line:
      	 p->startpos=(p->left_pos+p->right_pos)/2;
      	 p->curcmd=mot_line;
	p->motorspeed_l = mot.speedcmd;
	    p->motorspeed_r = mot.speedcmd;
       break;
   ...
   switch (p->curcmd){
    ...
    case mot_line: // Follows Black line until distance has been met or the black line is lost
	if ((p->right_pos+p->left_pos)/2- p->startpos > p->dist|| (double)CalLineLowest(linesensor) > lineBlack + 10){

		printf("%d\n", CalLineLowest(linesensor));          
		p->finished=1;
		p->motorspeed_l=0;
		p->motorspeed_r=0;
	}else{
		int K = 10;
		double offset_from_center = 3.5 - (double)CalLineLowest(linesensor);
		if(offset_from_center > -1 && offset_from_center < 1) {
			printf("%d\n", CalLineLowest(linesensor));
			p->motorspeed_l = mot.speedcmd;
			p->motorspeed_r = mot.speedcmd;
			...
			p->motorspeed_l -= offset_from_center/K;
			p->motorspeed_r = mot.speedcmd; 
		}else if(offset_from_center < -1) {
			printf("Dec Left\n");
			printf("%d\n", CalLineLowest(linesensor));
			// Deccelerate left motor
			p->motorspeed_r -= offset_from_center/K;
			p->motorspeed_l = mot.speedcmd; 
		  }
	       }
     break;
...
int calibratedLine(int value){
  int c;
  c = lineBlack + (value*(lineWhite - lineBlack))/128;
  return c;
}


int CalLineLowest(symTableElement *s){
  int low = lineWhite;
  int pos;
  for (int i = 0;i < s->length;i++){
	int val = calibratedLine(s->data[i]); 
	if(low > val){
	low = val;
	pos = i;
	}
