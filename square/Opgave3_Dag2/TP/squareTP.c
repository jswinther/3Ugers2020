...
*/
   ...
   if(count_31 < 1000 && (missionTime % 2) == 0){
   	samp.time = missionTime;
   	samp.r = mot.motorspeed_r;
   	samp.l = mot.motorspeed_l;
...
for(int i = 0; i < 1000; i++){
	...
	fprintf(f, "%d %f %f\n", log_31[i].time, log_31[i].r, log_31[i].l);
}
fclose(f);
...
void update_odo(odotype *p)
{
  ...
  U = (Ur + Ul)/2;
  delTheta = (Ur - Ul)/WHEEL_SEPERATION;

  p->theta = theta + delTheta;
