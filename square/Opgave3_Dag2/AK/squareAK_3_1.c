...
*/
  sm_update(&mission);
   ...
   if(missoncounter == 100)
   {
      ...
      Datalog[logcount++] = mot.motorspeed_l;
      Datalog[logcount++] = mot.motorspeed_r;
...
*/
  ...
  LOG_D = fopen("logfile.dat","w");    
  int i;
  for (i=0;i<missontimer*3;i=i+3)
  {
    fprintf(LOG,"%.f %.02f %.02f\n",Datalog[i],Datalog[i+1],Datalog[i+2]);
    fprintf(LOG_D,"%.f %.02f %.02f\n",Datalog[i],Datalog[i+1],Datalog[i+2]);
  }
