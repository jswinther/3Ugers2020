...
*/
   ...
   if(count_31 < 2500 && (missionTime % sampleRate) == 0){
   ...
   switch (p->curcmd){
     ...
     case mot_move:
       ...
       if ((p->right_pos+p->left_pos)/2- p->startpos > p->dist){
          ...
          double K = 0.005;
          double offset = odo.theta_old - odo.theta;
          // Deaccelerate right wheel.
          if(offset < -0.002) {
            printf("Deaccel Right Wheel\n");
            p->motorspeed_l = p->speedcmd;
            p->motorspeed_r -= K * fabs(offset);
          // Deaccelerate left wheel.
          } else if(offset > 0.002) {
            printf("Deaccel Left Wheel\n");
            p->motorspeed_r = p->speedcmd;
            p->motorspeed_l -= K * fabs(offset);
          } else {
            p->motorspeed_l=p->speedcmd;
            p->motorspeed_r=p->speedcmd;
          }
