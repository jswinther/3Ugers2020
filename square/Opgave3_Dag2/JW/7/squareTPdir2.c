...
void center_of_mass(motiontype *p, symTableElement *s) {
    ...
    for (int i = 0; i < 4; i++)
    {
        //printf("Sum of %d %d %d is %d", i, i+1, i+2, (s->data[i] + s->data[i+1] + s->data[i+2]));
        if((s->data[i] + s->data[i+1] + s->data[i+2] + s->data[i+3] + s->data[i+4]) < min) {
            min = (s->data[i] + s->data[i+1] + s->data[i+2] + s->data[i+3] + s->data[i+4]);
            center_of_mass = i+2;
        }
...
printf("center of mass = %d\n", center_of_mass);
  ...
  double offset_from_center = 3.5 - center_of_mass;
  //printf("Offset %f\n", offset_from_center);
  
  if(offset_from_center >= -0.5 && offset_from_center <= 0.5) {
    //printf("Center\n");
    p->motorspeed_l = DEF_FWD;
    p->motorspeed_r = DEF_FWD;
    ...
    //printf("Dec Right\n");
    // Deccelerate right motor
    p->motorspeed_r -= fabs(offset_from_center) * 0.05;
    p->motorspeed_l = DEF_FWD; 
  } else if(offset_from_center < -0.5) {
    printf("Dec Left\n");
    // Deccelerate left motor
    p->motorspeed_l -= offset_from_center * 0.05;
    p->motorspeed_r = DEF_FWD; 
  }
  if(missionTime > 5000) {
    mission.state = ms_end;
